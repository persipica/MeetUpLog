...
class MeetupDatabase:
    ...
    def add_message(self, room_id: str, user_id: str, text: str, reply_to_message_id: int | None = None) -> int:
        now = datetime.now(timezone.utc).isoformat()
        with self.connect() as db:
            db.execute("INSERT OR IGNORE INTO chat_rooms(id, created_at, updated_at) VALUES(?,?,?)", (room_id, now, now))
            db.execute("UPDATE chat_rooms SET updated_at=? WHERE id=?", (now, room_id))
            cursor = db.execute("INSERT INTO chat_messages(room_id,user_id,text,reply_to_message_id,created_at) VALUES(?,?,?,?,?)",
                                (room_id, user_id, text, reply_to_message_id, now))
    ...
    def messages_after(self, room_id: str, message_id: int) -> list[ChatMessage]:
        with self.connect() as db:
            rows = db.execute("SELECT id,user_id,text,reply_to_message_id FROM chat_messages WHERE room_id=? AND id>? ORDER BY id",
                              (room_id, message_id)).fetchall()
        return [ChatMessage(message_id=row["id"], user_id=row["user_id"], text=row["text"], reply_to_message_id=row["reply_to_message_id"]) for row in rows]

    def context_messages(self, room_id: str, before_or_at: int, limit: int = 12) -> list[ChatMessage]:
        with self.connect() as db:
            rows = db.execute("SELECT id,user_id,text,reply_to_message_id FROM chat_messages WHERE room_id=? AND id<=? ORDER BY id DESC LIMIT ?",
                              (room_id, before_or_at, limit)).fetchall()
        rows = list(reversed(rows))
        return [ChatMessage(message_id=row["id"], user_id=row["user_id"], text=row["text"], reply_to_message_id=row["reply_to_message_id"]) for row in rows]

    def preferences(self, room_id: str) -> list[Preference]:
        with self.connect() as db:
            rows = db.execute("SELECT preference_json FROM preference_snapshots WHERE room_id=? ORDER BY user_id", (room_id,)).fetchall()
    ...
    def reset_room(self, room_id: str):
        with self.connect() as db:
            ...
            db.execute("DELETE FROM chat_analysis_progress WHERE room_id=?", (room_id,))
...
class MySQLMeetupDatabase:
    ...
    def add_message(self, room_id: str, user_id: str, text: str, reply_to_message_id: int | None = None) -> int:
        with self.connect() as db, db.cursor() as cursor:
            ...
            cursor.execute("INSERT INTO chat_messages(room_id,user_id,text,reply_to_message_id,created_at) VALUES(%s,%s,%s,%s,NOW())",
                           (room_id, user_id, text, reply_to_message_id))
            return int(cursor.lastrowid)
    ...
    def context_messages(self, room_id: str, before_or_at: int, limit: int = 12) -> list[ChatMessage]:
        with self.connect() as db, db.cursor() as cursor:
            cursor.execute("SELECT id,user_id,text,reply_to_message_id FROM chat_messages WHERE room_id=%s AND id<=%s ORDER BY id DESC LIMIT %s",
    ...
    def preferences(self, room_id: str) -> list[Preference]:
        with self.connect() as db, db.cursor() as cursor:
            ...
            rows = cursor.fetchall()
        return [Preference.model_validate(row["preference_json"] if isinstance(row["preference_json"], dict) else json.loads(row["preference_json"])) for row in rows]
    ...
    def reset_room(self, room_id: str):
        with self.connect() as db, db.cursor() as cursor:
            ...
            cursor.execute("DELETE FROM chat_analysis_progress WHERE room_id=%s", (room_id,))
