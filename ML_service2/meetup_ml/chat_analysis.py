...
def _detect_people(
    ...
    for person_type, names in known.items():
        for name in names:
            for token in re.findall(r"[0-9A-Za-z가-힣]{2,}", name):
                alias_owners.setdefault((person_type, _clean(token)), set()).add(name)
    ...
    for person_type, names in known.items():
        for name in names:
            ...
            # 공백을 제거한 전체 문장에서 부분 문자열을 찾으면
            # "감독이 만든" 안에서 "이만" 같은 가짜 인물이 생긴다.
            # 실제 표기 문자열이 존재할 때만 정확 일치로 인정한다.
            matched = (
                len(clean_name) >= 2
                and name.casefold() in folded_text
            )

            if not matched:
                for token in re.findall(r"[0-9A-Za-z가-힣]{2,}", name):
                    clean_token = _clean(token)
                    if len(alias_owners.get((person_type, clean_token), set())) != 1:
                        continue
                    token_start = folded_text.find(token.casefold())
                    token_end = token_start + len(token)
                    if token_start >= 0 and any(
                        0 <= role_start - token_end <= 6
    ...
    # 띄어쓰기/한 글자 오타가 있는 인물명도 카탈로그 인물과 연결한다.
    message_tokens = [
    ...
    for person_type, names in known.items():
        for name in names:
            ...
            if any(
                len(_clean(token)) >= 4
                and _fuzzy_score(_clean(token), clean_name) >= 0.88
...
def _term_forms(token: str) -> set[str]:
    """토큰에서 조사/어미를 뗀 후보 어간들을 모은다.

    한 겹만 벗기면 "공포영화는"(장르+"영화"+조사"는"이 띄어쓰기 없이
    붙은, 실제 채팅에서 아주 흔한 형태) 같은 토큰이 "공포영화"까지만
    벗겨지고 "공포"까지는 못 벗겨져서 장르 인식에서 통째로 빠지는 문제가
    있었다 (반면 "나는 공포영화 싫어해"처럼 "영화"가 토큰 맨 끝이면 한
    겹만으로도 벗겨져서 정상 인식됐다 - 붙어 있는 조사 개수에 따라
    인식 여부가 갈리는 버그였다). _KOREAN_ENDINGS를 최대 2겹까지 반복
    적용해 "공포영화는" -> "공포영화" -> "공포"까지 도달하게 한다.
    """
    forms = {token}
    frontier = {token}

    for _ in range(2):  # "...영화는"처럼 조사가 2겹까지 붙는 경우를 커버
...
def _apply_effect(
    ...
    if kind == "GENRE":
        ...
        if value > 0:
            if target not in state["liked_people"]:
                state["liked_people"].append(target)
            if target in state["disliked_people"]:
                state["disliked_people"].remove(target)
        elif value < 0:
            if target not in state["disliked_people"]:
                state["disliked_people"].append(target)
            if target in state["liked_people"]:
                state["liked_people"].remove(target)
...
def analyze_chat(
    ...
    for message in request.messages:
        ...
        if agrees and agreement_effects:
            for inherited_effect in agreement_effects:
                ...
                analyses.append(
                    MessageAnalysis(
                        ...
                        confidence=0.82,
                        note="직전 의견의 복수 성향에 동의하여 본인 성향으로 반영",
                    )
                )
            ...
            last_effect = agreement_effects[-1]
            last_effects = agreement_effects
        ...
        if inline_agreement and agreement_effect:
            ...
            _apply_effect(state, agreement_effect, movies)
            analyses.append(
                MessageAnalysis(
                    user_id=message.user_id,
                    text=message.text,
        ...
        person_results: list[tuple[str, str, float]] = []
        ...
        if detected_people:
            ...
            for person_name, _person_type in detected_people:
                ...
                person_attitude, person_score = (
                    _local_preference_attitude(matching_clause)
                )
                ...
                person_results.append(
                    (person_name, person_attitude, person_score)
                )

                if person_score > 0:
                    if person_name not in state["liked_people"]:
                        state["liked_people"].append(person_name)
                    if person_name in state["disliked_people"]:
                        state["disliked_people"].remove(person_name)
                else:
                    if person_name not in state["disliked_people"]:
                        state["disliked_people"].append(person_name)
                    if person_name in state["liked_people"]:
                        state["liked_people"].remove(person_name)
            ...
            if not movie_title and person_results:
                ...
                for person_name, person_attitude, person_score in person_results:
                    analyses.append(
                        MessageAnalysis(
                            user_id=message.user_id,
                            text=message.text,
                ...
                last_effects = [
                    ("PERSON", name, score, label)
                    for name, label, score in person_results
                ]
                if recent_intent:
                    analyses.append(
                        MessageAnalysis(
        ...
        if korean_only:
            ...
            if comparative_country_match:
                less_preferred = COUNTRY_ALIASES[comparative_country_match.group(1)]
                preferred = COUNTRY_ALIASES[comparative_country_match.group(2)]
